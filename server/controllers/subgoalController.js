    ...
    async.parallel(
      ...
      function(err, results) {
        ...
        for (let i = 0; i < filteredGoals.length; i++) {
          filteredGoals[i].title =
            filteredGoals[i].title.charAt(0).toUpperCase() +
            filteredGoals[i].title.slice(1);

          if (filteredGoals[i].description.length > MAX_DESCRIPTION_LENGTH)
            filteredGoals[i].description =
              filteredGoals[i].description.substring(
                0,
...
exports.create_goal_post = [
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if (req.file) {
        // Image provided during goal creation
        try {
